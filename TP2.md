...
1. TRAPHANDLER pushea un número de trap en el stack y luego salta a _alltraps (etiqueta). Se debe usar donde la CPU pushea un código de error automáticamente.
 ...
 Si siempre se utilizara TRAPHANDLER se esperaría que el CPU siempre haga push del código de error; pero como esto no sucede siempre, se dejaría de respetar el struct definido (el número de trap se tomaría como error code, por ejemplo) generando múltiples inconvenientes.
